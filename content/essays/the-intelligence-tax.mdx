...
I have spent over a decade building enterprise AI — first at Google, where I was a founding member of Google Cloud AI, then building and selling a company, now leading AI strategy at Uniphore. I have watched hundreds of enterprise deployments from the inside. The pattern I see now is one I have seen before in cloud, in SaaS, in every platform shift: the organizations that rent capability get started faster, but the ones that own the layer that matters build the durable advantage.
...
The self-hosted option is 14× cheaper than GPT-5.5 and 41× cheaper than Opus. The breakeven against GPT-5.5 is roughly 6,000 interactions per day. Against Opus, it is 2,500. Any enterprise running agents at production volume is well past both thresholds.
...
The broader market confirms this at a different scale. OpenRouter publishes real-time usage rankings — the closest thing to a revealed-preference survey of what developers actually deploy. As of June 2026, the five most-used models on the platform are all open-weight:

1. DeepSeek V4 Flash — 4.51 trillion tokens served
2. Tencent Hy3 Preview — 4.26 trillion tokens
3. MiniMax M3 — 4.15 trillion tokens, growing 103% month over month
4. Xiaomi MiMo-V2.5 — 3.43 trillion tokens
5. Owl Alpha — 2.43 trillion tokens

The first US closed model — Claude Opus 4.7 — appears at number six.

The benchmark data explains why. DeepSeek V4 Pro Max scores 80.6% on SWE-bench Verified — tied with Gemini 3.1 Pro, the best closed model on that benchmark — at $0.87 per million output tokens. Claude Opus 4.8 costs $25. GPT-5.5 costs $30. MiniMax M3 scores 80.5% on the same benchmark at $1.20 per million output. These are not approximations of frontier performance at lower cost. On the benchmarks that matter most for production agents — real code, real repositories, real engineering tasks — the open-weight models are matching the frontier.
...
This is the flywheel. Curate domain knowledge. Train or fine-tune a model. Deploy on your infrastructure. Collect production feedback. Improve the model. Redeploy. Each cycle makes the model more capable and the cost advantage wider. The rental customer pays the same per-token price on day one and day one thousand. The owner's effective cost per unit of intelligence drops with every cycle.
...
Frontier models will keep getting more powerful. They will also keep getting more controlled, more retained, and more restricted. The question is not whether your current model is good enough. The question is whether your AI strategy survives the next export directive, the next retention policy, the next pricing revision — or whether it depends entirely on decisions made in rooms you are not in.

---

*Will Lu is VP Engineering & Head of AI Strategy at Uniphore. Previously, he was a founding member of Google Cloud AI and co-founded Orby AI, which was acquired by Uniphore. Stanford GSB.*
